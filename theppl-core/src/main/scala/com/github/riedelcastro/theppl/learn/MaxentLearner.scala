...
trait MaxentLearner2[Context] extends Learner[Context] with HasLogger {
  ...
  def expectator(potential: template.PotentialType): Expectator = Expectator(potential)
  ...
  def train() {
    ...
    logger.info("Extracting gold features.")
    val goldFeatures: Seq[ParameterVector] = potentials.map(_._2.targetExpectations)
