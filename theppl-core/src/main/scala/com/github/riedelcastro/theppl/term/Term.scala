...
object Not extends Constant((x: Boolean) => !x) with UnaryFun[Boolean, Boolean] {
  def symbol = "!"
...
object Or extends Constant((x: Boolean, y: Boolean) => x || y) with InfixFun[Boolean, Boolean, Boolean] {
  def symbol = "||"
}
...
//object VecImplies extends Constant((x: Vec, y: Vec) => {
//  xa = x.activeIndices
//}) with InfixFun[Vec, Vec, Vec] {
//  def symbol = "v=>"
//}
...
object ParameterVectorAddN
  extends Constant((args: Seq[ParameterVector]) => args.fold(new ParameterVector)((l, r) => {
    l += r
  })) {
...
object LogicPlayground extends TermImplicits {
  def main(args: Array[String]) {
    ...
    val test3 = forall {
      for (x <- Person) yield smokes(x)
    }
    println(test3)
    println(forall {
      for (x <- Person) yield And(cancer(x), smokes(x))
    })
    println(forall {
      for (x <- Person) yield cancer(x) && smokes(x)
    })
    println(forall {
      for (x <- Person) yield forall {
        for (y <- Person) yield cancer(x) |=> smokes(y)
      }
    })
    println(forall {
      for (x <- Person; y <- Person) yield friends(x, y) && friends(y, x)
    })
    println(forall {
      for (x <- Person; y <- Person) yield friends(x, y) && friends(y, x)
    })
    println(forall {
      for (x <- Person; y <- Person) yield friends(x + 1, 1 + 1 + y) === friends(y, x)
    })
    println(forall {
      for (x <- Person; y <- Person) yield (x + y) === (y + x)
    })
    println(sum {
      for (x <- Person; y <- Person) yield I((x + y) === (y + x))
    })
