...
import com.github.riedelcastro.theppl.util.{ArrayImports, HasLogger}
...
class GradientAscent(val optimizable: OptimizableByValueAndGradient) extends Optimizer with HasLogger {
  ...
  def optimize(numIterations: Int = Int.MaxValue): Boolean = {
    ...
    for (iteration <- 0 until numIterations) {
      ...
      val gg = ArrayImports.twoNorm(gradient)
